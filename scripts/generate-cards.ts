...
const SYNC_ASSETS_ONLY = process.argv.includes('--sync-assets') || process.argv.includes('--sync');
 ...
 */
async function getGreatJobPrompts(content: string, type: 'letter' | 'digraph' | 'word' | 'cvc' | 'sentence'): Promise<string[]> {
...
/**
 * Regenerate asset maps from filesystem
 * This can be run independently to sync asset maps with whatever is on disk
 */
function regenerateAssetMapsFromFilesystem(): void {
  console.log('\n📁 Regenerating asset maps from filesystem...');
 ...
 * This maps asset paths to their require() statements.
 * 
 * Generated from actual files in assets/${LOCALE} directory
 * DO NOT EDIT THIS FILE MANUALLY
 */
...
if (require.main === module) {
  // If --sync-assets flag is provided, just regenerate asset maps from filesystem
  if (SYNC_ASSETS_ONLY) {
    console.log('🔄 Syncing asset maps with filesystem (no card generation)...\n');
    regenerateAssetMapsFromFilesystem();
    console.log('\n✅ Asset maps synchronized with filesystem!');
    console.log('💡 Restart your Metro bundler with: npm run start:clear');
    process.exit(0);
  }
  
  main()
    .catch((error) => {
      console.error('\n❌ Error during card generation:', error.message);
      // Even on error, try to regenerate asset maps from whatever is on disk
      console.log('\n⚠️  Attempting to regenerate asset maps from existing files...');
      try {
        regenerateAssetMapsFromFilesystem();
      } catch (e) {
        console.error('Failed to regenerate asset maps:', e);
      }
      process.exit(1);
    });
}
